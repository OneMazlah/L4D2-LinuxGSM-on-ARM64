...
if [[ "${arch}" != "x86_64" && "${arch}" != "i386" && "${arch}" != "i686" ]]; then
	if [[ ("${arch}" == "aarch64" || "${arch}" == "arm64") && -n "$(command -v box86 2> /dev/null)" ]]; then
		fn_print_warn_nl "ARM64 host detected; continuing because box86 is available"
		fn_script_log_warn "ARM64 host detected; continuing because box86 is available"
	else
