...
fn_install_missing_deps() {
	fn_arm64_box86_filter_missing_deps
...
fn_check_loop() {
	# Loop through required dependencies and check whether they are installed.
...
fn_arm64_box86_adjust_deps() {
	if [[ "${arch}" != "aarch64" && "${arch}" != "arm64" ]]; then
		return
	fi

	if [ -z "$(command -v box86 2> /dev/null)" ]; then
		return
	fi
	...
	for dep in "${array_deps_required[@]}"; do
		case "${dep}" in
			steamcmd|lib32gcc-s1|lib32stdc++6|libsdl2-2.0-0:i386|bsdmainutils)
				continue
				;;
		esac
		adjusted_deps+=("${dep}")
	done
	array_deps_required=("${adjusted_deps[@]}")

	adjusted_deps=()
	for dep in "${array_deps_required_steamcmd[@]}"; do
		case "${dep}" in
			steamcmd|lib32gcc-s1|lib32stdc++6|libsdl2-2.0-0:i386|bsdmainutils)
				continue
				;;
	...
	array_deps_required_steamcmd=("${adjusted_deps[@]}")
}
...
fn_arm64_box86_filter_missing_deps() {
	...
	array_deps_missing=("${filtered_missing[@]}")
	unset steamcmdfail
}
